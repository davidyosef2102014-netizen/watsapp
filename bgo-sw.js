const CACHE = 'bgo-v3';
const SHELL = ['/watsapp/bgo-icon.svg'];
...
self.addEventListener('fetch', e => {
  ...
  if (u.hostname.includes('cartocdn')||u.hostname.includes('osrm')||u.hostname.includes('nominatim')||u.hostname.includes('photon')||u.hostname.includes('hivemq')||u.hostname.includes('unpkg')||u.hostname.includes('openstreetmap')) { e.respondWith(fetch(e.request).catch(()=>new Response('',{status:503}))); return; }
